...
def check_results_exist(report_dir: str) -> bool:
    """Check if results already exist for any probe in the report directory"""
    if not os.path.exists(report_dir):
        return False
    
    # Check if any probe directory has HTML report files
    for probe_dir in os.listdir(report_dir):
        probe_path = os.path.join(report_dir, probe_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--force_rerun", action="store_true", help="Force rerun even if results exist")
    ...
    if check_results_exist(report_dir) and not args.force_rerun:
        print(f"\nResults already exist. Delete report files at {args.output_basedir} if you want to rerun Garak.")
        print("Or use --force_rerun to force rerun all probes.")
        sys.exit(0)
